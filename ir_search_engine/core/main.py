...
def main():
    ...
    while True:
        ...
        print("6. Exit")
        print("7. Query Optimization (Pseudo-Relevance Feedback)")
        ...
        if model_choice == '5': 
            ...
            queries_for_eval = {q.query_id: q for q in queries} 

            qrels_for_eval = defaultdict(dict)
            for qrel_item in qrels:
        ...
        if model_choice == '7':
            ...
            if retrieval_model_name_for_prf == 'TF-IDF':
                retrieval_model = vsm_model
            elif retrieval_model_name_for_prf == 'BERT':
                retrieval_model = bert_model
            else:
                print("Invalid model choice for PRF. Aborting optimization.")
                continue
            ...
            if expanded_query != original_query:
                ...
                if perform_expanded_search == 'yes':
                    ...
                    if expanded_search_model_choice == '1':
                        ...
                        search_results = vsm_model.search(expanded_query, top_k=10)
                    elif expanded_search_model_choice == '2':
                        search_model_name = "BERT Embeddings"
                        search_results = bert_model.search(expanded_query, top_k=10)
                    elif expanded_search_model_choice == '3':
                        search_model_name = "Hybrid Search"
                        search_results = hybrid_ranker.hybrid_search(
                            expanded_query, 
                            top_k=10, 
                    ...
                    if search_results:
                        for i, (doc_id, score) in enumerate(search_results):
                            original_text = bert_model.documents_text.get(doc_id)
                            if original_text is None:
                                ...
                                if converted_doc_id is not None:
                                    original_text = bert_model.documents_text.get(converted_doc_id)
                            if original_text is None:
        ...
        if model_choice == '1':
            model_used = "TF-IDF"
            results = vsm_model.search(query_text, top_k=10)
        elif model_choice == '2':
            model_used = "BERT Embeddings"
            results = bert_model.search(query_text, top_k=10)
        elif model_choice == '3':
            model_used = "Hybrid Search"
            results = hybrid_ranker.hybrid_search(
                query_text, 
                top_k=10, 
                vsm_weight=0.1,                
                bert_weight=0.9,               
